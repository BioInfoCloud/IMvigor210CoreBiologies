...
# Read cd8-coordinate and perimeter data
barcode <- c("6A6C", paste("68A", LETTERS[22:26], sep=""), paste("68B", 0:9, sep=""), paste("68B", LETTERS[1:3], sep=""))
...
for(i in 1:length(barcode))
{
  ...
  wkbk <- loadWorkbook(file.path(fileDir,
    paste("20170525_MSR9228_cd3_666_coord_",barcode[i],".xlsx",sep="")))
  s <- length(sheets(wkbk))
  dat.1L0 = rbind(readWorkbook(wkbk, sheet = 1, colNames = FALSE), readWorkbook(wkbk, sheet = 2, colNames = FALSE))
  dat.1L1 = rbind(readWorkbook(wkbk, sheet = 3, colNames = FALSE), readWorkbook(wkbk, sheet = 4, colNames = FALSE))
  dat.1L2 = rbind(readWorkbook(wkbk, sheet = 5, colNames = FALSE), readWorkbook(wkbk, sheet = 6, colNames = FALSE))
  ...
  while(s > 9)
  {
    dat.2L0 = rbind(readWorkbook(wkbk, sheet = (sec - 1)*9 + 1, colNames = FALSE), readWorkbook(wkbk, sheet = (sec - 1)*9 + 2, colNames = FALSE))
    dat.2L1 = rbind(readWorkbook(wkbk, sheet = (sec - 1)*9 + 3, colNames = FALSE), readWorkbook(wkbk, sheet = (sec - 1)*9 + 4, colNames = FALSE))
    dat.2L2 = rbind(readWorkbook(wkbk, sheet = (sec - 1)*9 + 5, colNames = FALSE), readWorkbook(wkbk, sheet = (sec - 1)*9 + 6, colNames = FALSE))
...
wkbk <- loadWorkbook(file.path(fileDir,
  paste("./20170525_MSR9228_cd3_666_subset_var_Lx.xlsx",sep="")))
mice.info <- readWorkbook(wkbk, sheet = 10, colNames = TRUE)
...
mice.info.small <- mice.info.small[which(mice.info.small$Antibody == "CD3*" & mice.info.small$Barcode %in% barcode),]
...
dat19 <- hyperframe(Barcode=mice.info.small$Barcode[which(mice.info.small$Barcode == barcode[i])],
...
## dat19 included in package
#save(dat19, file = paste("dat19.RData",sep=""))

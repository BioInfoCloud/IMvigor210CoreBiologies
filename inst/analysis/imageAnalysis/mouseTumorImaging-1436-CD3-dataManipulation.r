...
barcode <- c(paste("4MT", LETTERS[5:26], sep=""), paste("4MU", 0:2, sep=""))
...
for(i in 1:length(barcode))
{
  ...
  wkbk <- loadWorkbook(file.path(fileDir,
    paste("20170515_MSR9005_cd3_1436_Lx_coord_",barcode[i],".xlsx",sep="")))
...
wkbk <- loadWorkbook(file.path(fileDir,
  paste("./20170515_MSR9005_cd3_1436_subset_fix_Lx.xlsx",sep="")))
...
mice.info.small <- mice.info.small[which(mice.info.small$Antibody == "CD3*" & mice.info.small$Treatment != "\t"),]
...
dat25 <- hyperframe(Barcode=mice.info.small$Barcode[which(mice.info.small$Barcode == barcode[i])],
...
## dat25 included in package
#save(dat25, file = paste("dat25.RData",sep=""))
